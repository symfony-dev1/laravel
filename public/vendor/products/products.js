$(document).ready(function ($) {
    ...
    $("#terms").on('click', '.addTerm', function (event) {
        ...
        if ($.isNumeric(term_id)) {
            if ($("select.attribute_terms option[value='" + term_id + "']").is(':disabled')) {
                ...
                html += '<span  class="removeTerm text-red ml-2" id="' + term_id + '"><span id="' + term_id + '" class="removeTerm badge badge-light" ><input name="attribute_terms[]" type="hidden" data-attribute_id="' + attribute_id + '" data-term_id="' + term_id + '" class="' + attribute_id + ' all_attribute_terms" id="' + term_id + '" value="' + term_id + '">' + term_name + '</span>x</span>'
    ...
    function updateVAriations() {
        ...
        $.each(inputs, function (key, value) {
            var attribute_id = $(value).data("attribute_id");
            var term_id = $(value).data("term_id");
            attribute_ids.push(attribute_id);
            term_ids.push(term_id);
        });
        if (attribute_ids.length != 0 && term_ids.length != 0) {
            $.ajaxSetup({
                headers: {
            ...
            $.ajax({
                ...
                data: { "attribute_ids": attribute_ids, "term_ids": term_ids },
                success: function (response) {
                    console.log(response)
                    if (response != 'empty') {
                        var html = '<div class="d-flex">';
                        $.each(response, function (key, value) {
                            html += '<select width="70px" id="select_attribute_' + value.attribute_id + '" name="select_attribute_' + value.attribute_id + '" data-attribute_id="' + value.attribute_id + '" data-attribute_slug="' + value.attribute_slug + '" class="ml-2 form-control productAttribute">'
                            html += '<option value="any">--Any ' + value.attribute_slug + '--</option>'
                            $.each(value.terms, function (key1, value1) {
                                html += '<option value="' + value1.term_id + '">' + value1.term_slug + '</option>'
                            });
                            html += '</select>'
                        });
                        html += '<button class="ml-2 btn btn-primary addVariant">add</button></div>'

                        console.log(html)
                        $('#selected_attributes_and_terms').html(html);
                    } else {
                        html = $('#selected_attributes_and_terms').html('');
    ...
    var selectedVariant = [];
    $("#selected_attributes_and_terms").on('click', '.addVariant', function (event) {
        event.preventDefault();
        ...
        var value = "";
        html = '';
        $('#selected_attributes_and_terms select.productAttribute').each(function () {
            html += '<input type="hidden" name="' + $(this).data("attribute_slug") + '[]" value="' + $(this).val() + '"></input>';
            text += $(this).data("attribute_slug") + $(this).val();
            value += $(this).val();
            var attribute_name = $(this).data("attribute_slug")
            var term_name = $("#selected_attributes_and_terms select#select_attribute_" + $(this).data("attribute_id") + " option:selected").text()
            variant_label += attribute_name + ": " + term_name + "&nbsp;&nbsp";
        })
        if ($.inArray(text, selectedVariant) !== -1) {
            alert("This variant already exists!")
            return
        } else {
            selectedVariant.push(text);
        }
        ...
        html += '<h6 class="font-weight-bold">' + variant_label + '</h6>';
        html += '<div class="row">' +
            '<div class="col-md-10">' +
    ...
    $("#variants").on('click', '.removeVariantBtn', function (event) {
        ...
        if (confirmResult) {
            ...
            $('.' + arr[0]).remove();
            $("#" + arr[0]).remove();
            selectedVariant.splice($.inArray(arr[0], selectedVariant), 1);
